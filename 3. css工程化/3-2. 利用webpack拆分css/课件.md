...
课前问题：细分js，然后不用担心 webpack搞定。那对于css，webpack是如何处理的呢？
...
【打印require样式文件之后会打印出一个数组，不过我们一般不会关心导出的对象】
...
```js
var import1 = __webpack_require__("./src/bg.png"); // 这里需要配合file-loader/url-loader替换成一个路径/base64
// 把其它模块的处理结果放这，css-loader不处理这里__webpack_require__返回的结果
module.exports = `.red{
    color:"#f40";
    background:url("${import1}")
}`;
...
一个样式文件被导入一次，loader处理一次，然后保存起来。下一次再次导入这个文件，就不处理了，所以loader对一个文件只会处理一次。
...
module.exports = {}
```
// 生成style标签，插入到页面中，所以叫style-loader
